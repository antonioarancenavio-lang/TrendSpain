...
function TarjetaOportunidad({ trend, onClick, isFavorite, onToggleFavorite, tieneAcceso }) {
  ...
  return (
    <div onClick={onClick} className="trend-card" style={{
      position: "relative",
      overflow: "hidden",
      ...
      <div className="trend-card-glow" style={{
        position:"absolute", inset:0, pointerEvents:"none",
        background: `radial-gradient(circle at 85% -10%, ${C.gold}22, transparent 55%)`,
        opacity: 0,
      }} />

      {/* borde superior dorado sutil */}
      <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background: `linear-gradient(90deg, ${C.gold}66, transparent 70%)` }} />

      {/* favorito */}
      <button
        onClick={(e) => { e.stopPropagation(); onToggleFavorite(trend.id); }}
        style={{ position:"absolute", top:14, right:14, zIndex:20, fontSize:18, background:"none", border:"none", cursor:"pointer", padding:0 }}
        aria-label="favorito"
      >
        {isFavorite ? "❤️" : "🤍"}
      </button>
      ...
      <div style={{ position:"relative", zIndex:10, display:"flex", flexDirection:"column", alignItems:"center", textAlign:"center", padding:"36px 24px", gap:14 }}>
        <div className="trend-card-emoji" style={{
          fontSize: 40,
          width: 76, height: 76,
        ...
        <h3 className="trend-card-title" style={{ fontSize:17, fontWeight:800, lineHeight:1.35, letterSpacing:"-0.01em", color: C.text, margin:0 }}>
          {trend.nombre}
        </h3>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", justifyContent:"center" }}>
          <span style={{
            ...
            background:"rgba(255,255,255,0.04)", color:C.textMuted, border:`1px solid ${C.border}`,
          }}>
            {trend.sector}
          </span>
          <span style={{
            fontSize:10, fontWeight:800, textTransform:"uppercase", letterSpacing:"0.12em",
            padding:"4px 10px", borderRadius:99,
            background: pot.bg, color: pot.color, border:`1px solid ${pot.border}`,
          }}>
            🔥 {trend.potencial}
          </span>
        </div>
      ...
      <div className="trend-card-cta" style={{
        position:"relative", zIndex:10, textAlign:"center", paddingBottom:16,
    ...
    <div style={{ position:"fixed", inset:0, zIndex:100, background:"rgba(10, 10, 12, 0.96)", backdropFilter:"blur(14px)", overflowY:"auto" }}>
      <div style={{ maxWidth:860, margin:"0 auto", padding:"30px 20px 100px" }}>
        ...
        <div style={{
          ...
          <div className="hero-flex" style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:20, position:"relative", zIndex:1 }}>
        ...
        {/* ── MÉTRICAS ── */}
        <div className="metric-grid" style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(160px, 1fr))", gap:14, marginBottom:32 }}>
          {[
        ...
        <div style={{ display:"grid", gap:20 }}>
          ...
          <div className="metric-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:55, background:"rgba(0,0,0,0.82)", display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:20, width:"100%", maxWidth:720, maxHeight:"90vh", overflowY:"auto", padding:"24px 20px" }}>
        ...
        {tA && tB && (
          <div className="compare-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, background:C.card, borderRadius:16, padding:16, border:`1px solid ${C.border}` }}>
            <div>
...
export default function App() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"-apple-system, 'Segoe UI', system-ui, sans-serif", color:C.text }}>
      <style>{`
        .trend-card { transition: transform 0.25s ease, box-shadow 0.25s ease; }
        .trend-card:hover { transform: translateY(-4px); box-shadow: 0 16px 32px -12px rgba(0,0,0,0.5); }
        .trend-card:hover .trend-card-glow { opacity: 1; transition: opacity 0.4s ease; }
        .trend-card:hover .trend-card-emoji { transform: scale(1.08) rotate(3deg); transition: transform 0.25s ease; }
        .trend-card:hover .trend-card-title { color: ${C.goldLight}; transition: color 0.25s ease; }
        .trend-card:hover .trend-card-cta { color: ${C.goldLight}; transition: color 0.25s ease; }
