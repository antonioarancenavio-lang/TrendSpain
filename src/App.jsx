    ...
    <div style={{minHeight:"100vh",background:"#f8f8fc",fontFamily:"system-ui,-apple-system,sans-serif"}}>
      <div style={{background:"white",borderBottom:"1px solid #e5e7eb",padding:"20px 16px 0"}}>
        <div style={{maxWidth:640,margin:"0 auto"}}>
          ...
          <p style={{fontSize:13,color:"#9ca3af",margin:"0 0 16px 34px"}}>Negocios que triunfan en EE.UU. y aún no han llegado a España</p>
      ...
      <div style={{maxWidth:640,margin:"0 auto",padding:16}}>
        <p style={{fontSize:13,color:"#9ca3af",marginBottom:12}}><span style={{fontWeight:700,color:"#111827"}}>{filtered.length}</span> oportunidades · toca una para ver el análisis completo</p>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:12}}>
          {filtered.map(t=>(
            <button key={t.id} onClick={()=>setSelected(t)}
              style={{textAlign:"left",background:"white",border:"1px solid #e5e7eb",borderRadius:14,padding:18,cursor:"pointer",transition:"all 0.15s"}}
      ...
      <Modal t={selected} onClose={()=>setSelected(null)}/>
    </div>
  );
}
