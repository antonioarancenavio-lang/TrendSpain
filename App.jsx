...
// ─── CONFIGURACIÓN ──────────────────────────────────────────────
// 1. Reemplaza esta URL por tu Payment Link real de Stripe (sin test_)
// 2. Reemplaza SECRET_TOKEN por una cadena aleatoria larga que solo tú conozcas
//    Ejemplo: "ts_unlock_x9k2m8p4q7" — la pones también en Stripe como ?token=ts_unlock_x9k2m8p4q7
const STRIPE_PAYMENT_LINK = "https://buy.stripe.com/TU_LINK_REAL_AQUI";
const SECRET_TOKEN = "ts_unlock_2025";
const FREE_TOKENS = 2; // Fichas gratuitas que puede abrir el usuario
// ────────────────────────────────────────────────────────────────
...
function PaywallModal({onClose}){
  const stripeUrl = `${STRIPE_PAYMENT_LINK}?client_reference_id=${SECRET_TOKEN}&success_url=${encodeURIComponent(window.location.origin + window.location.pathname + "?token=" + SECRET_TOKEN)}`;
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:60,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"white",borderRadius:24,padding:28,width:"100%",maxWidth:400,textAlign:"center",boxShadow:"0 25px 50px rgba(0,0,0,0.25)"}}>
        <div style={{fontSize:52,marginBottom:8}}>🚀</div>
        <h2 style={{fontSize:22,fontWeight:800,color:"#111827",margin:"0 0 6px"}}>TrendSpain Pro</h2>
        <p style={{fontSize:14,color:"#6b7280",lineHeight:1.6,margin:"0 0 4px"}}>
          Accede a las <strong style={{color:"#111827"}}>{TRENDS.length} oportunidades completas</strong>
        </p>
        <p style={{fontSize:28,fontWeight:800,color:"#6366f1",margin:"12px 0"}}>
          5€<span style={{fontSize:14,fontWeight:500,color:"#9ca3af"}}>/mes</span>
        </p>
        <div style={{background:"#f9fafb",borderRadius:14,padding:16,marginBottom:20,textAlign:"left"}}>
          {[
            "✅ Acceso a las 24 oportunidades",
            "✅ Análisis completo de cada negocio",
            "✅ Por qué funciona en EE.UU.",
            "✅ Cómo aplicarlo paso a paso en España",
            "✅ Riesgos y primeros pasos detallados",
            "✅ Nuevas oportunidades cada mes",
            "✅ Cancela cuando quieras"
          ].map((item,i)=>(
            <p key={i} style={{fontSize:13,color:"#374151",margin:"0 0 6px",fontWeight:500}}>{item}</p>
          ))}
        </div>
        <a href={stripeUrl} target="_blank" rel="noopener noreferrer"
          style={{display:"block",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"white",padding:16,borderRadius:14,fontSize:16,fontWeight:800,textDecoration:"none",marginBottom:10,boxShadow:"0 4px 15px rgba(99,102,241,0.4)"}}>
          💳 Suscribirme ahora — 5€/mes
        </a>
        <p style={{fontSize:11,color:"#9ca3af",margin:"0 0 10px"}}>Pago seguro con Stripe · Cancela cuando quieras</p>
...
function SuccessModal({onClose}){
  return(
    <div style={{position:"fixed",inset:0,zIndex:70,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:"white",borderRadius:24,padding:32,width:"100%",maxWidth:360,textAlign:"center"}}>
        <div style={{fontSize:56,marginBottom:12}}>🎉</div>
        <h2 style={{fontSize:22,fontWeight:800,color:"#111827",margin:"0 0 8px"}}>¡Bienvenido a Pro!</h2>
        <p style={{fontSize:14,color:"#6b7280",lineHeight:1.6,margin:"0 0 20px"}}>
          Ya tienes acceso completo a las {TRENDS.length} oportunidades de negocio.
        </p>
        <button onClick={onClose}
          style={{width:"100%",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"white",border:"none",borderRadius:14,padding:16,fontSize:16,fontWeight:800,cursor:"pointer"}}>
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:50,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"white",width:"100%",maxWidth:560,borderRadius:"20px 20px 0 0",padding:"24px 20px 40px",overflowY:"auto",maxHeight:"88vh"}}>
        ...
        <button onClick={()=>{onClose();onPaywall();}} style={{width:"100%",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"white",border:"none",borderRadius:12,padding:14,fontSize:14,fontWeight:700,cursor:"pointer"}}>
...
export default function App(){
  ...
  const [tokensLeft,setTokensLeft]=useState(()=>{
    ...
    return saved !== null ? parseInt(saved) : FREE_TOKENS;
  });

  // Detectar token en URL al cargar (vuelta de Stripe) o en sessionStorage
  useEffect(()=>{
  ...
  const handleCard=(t)=>{
    ...
    const newTokens = tokensLeft - 1;
    setTokensLeft(newTokens);
    sessionStorage.setItem("ts_tokens_left", newTokens);
    setSelected(t);
  };
    ...
    <div style={{minHeight:"100vh",background:"#f8f8fc",fontFamily:"system-ui,-apple-system,sans-serif"}}>
      ...
      <div style={{background:"white",borderBottom:"1px solid #e5e7eb",padding:"20px 16px 0"}}>
        <div style={{maxWidth:640,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
            ...
            {isPro
              ? <span style={{background:"#dcfce7",color:"#16a34a",fontSize:12,fontWeight:700,padding:"6px 12px",borderRadius:99}}>✅ Pro activo</span>
              : <button onClick={()=>setShowPaywall(true)} style={{background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"white",border:"none",borderRadius:10,padding:"8px 14px",fontSize:12,fontWeight:700,cursor:"pointer",boxShadow:"0 2px 8px rgba(99,102,241,0.3)"}}>
                  🔓 Pro — 5€/mes
                </button>
            }
          </div>
          ...
          {/* Banner freemium (solo si no es pro) */}
          {!isPro && (
            <div style={{background: tokensLeft > 0 ? "linear-gradient(135deg,#eef2ff,#f0f9ff)" : "linear-gradient(135deg,#fff1f2,#fff7ed)", border: tokensLeft > 0 ? "1px solid #c7d2fe" : "1px solid #fecaca", borderRadius:12,padding:"10px 14px",marginBottom:14,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
              <div>
                <p style={{fontSize:12,color: tokensLeft > 0 ? "#3730a3" : "#dc2626",margin:0,lineHeight:1.4,fontWeight:600}}>
                  {tokensLeft > 0
                    ? <>🎫 Te quedan <strong>{tokensLeft} ficha{tokensLeft!==1?"s":""} gratis</strong> de {FREE_TOKENS}</>
                    : <>🔒 Has usado tus {FREE_TOKENS} fichas gratuitas</>
                  }
                </p>
                {tokensLeft > 0 && <p style={{fontSize:11,color:"#6366f1",margin:"2px 0 0",fontWeight:500}}>Cada ficha abre el análisis completo de 1 negocio</p>}
              </div>
              <button onClick={()=>setShowPaywall(true)} style={{background:"#6366f1",color:"white",border:"none",borderRadius:8,padding:"6px 12px",fontSize:11,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>
                Ver todo
              </button>
            </div>
          )}
      ...
      <div style={{maxWidth:640,margin:"0 auto",padding:16}}>
        <p style={{fontSize:13,color:"#9ca3af",marginBottom:12}}>
          ...
          {!isPro && tokensLeft > 0 && <> · <span style={{color:"#6366f1",fontWeight:600}}>🎫 {tokensLeft} ficha{tokensLeft!==1?"s":""} disponible{tokensLeft!==1?"s":""}</span></>}
          {!isPro && tokensLeft === 0 && <> · <span style={{color:"#ef4444",fontWeight:600}}>🔒 Sin fichas — hazte Pro</span></>}
        </p>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:12}}>
          {filtered.map((t,i)=>{
            const exhausted = !isPro && tokensLeft === 0;
            return(
              <button key={t.id} onClick={()=>handleCard(t)}
                style={{textAlign:"left",background:"white",border:"1px solid #e5e7eb",borderRadius:14,padding:18,cursor:"pointer",transition:"all 0.15s",position:"relative"}}
                ...
                {!isPro && <div style={{position:"absolute",top:10,right:10,background: exhausted?"#fee2e2":"#eef2ff",borderRadius:99,padding:"2px 8px",fontSize:10,fontWeight:700,color: exhausted?"#dc2626":"#6366f1"}}>{exhausted?"🔒 Pro":"🎫 -1 ficha"}</div>}
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                  <div style={{paddingRight:50}}>
                    ...
                    <h3 style={{fontSize:14,fontWeight:700,color:"#111827",margin:0,lineHeight:1.3}}>{t.nombre}</h3>
                  </div>
                  <span style={{fontSize:24,flexShrink:0,marginLeft:8}}>{t.emoji}</span>
                </div>
